...
一条续写指令从输入到输出经过 9 个阶段，下面是各节点当前的打通状态。这是一份活文档，每轮 iter 收官时同步。最近一次更新：**iter 078**（2026-07-05，收官）——**六方审查 P1 修复批（8 组）+ 技债 2 项**：承接 iter077 P0 后的 P1 清单，本轮清掉 capstone 前大部分人工规避项：① reviewer 分数护栏（NaN/Inf/bool 拒收，纯数字字符串 coerce，判定处留 `score_warning`）；② workspace 写锁（CLI/Web/auto-pipeline 写章入口 flock 互斥，holder 回显）；③ rolling summary 落盘顺序改 persist→rolling，并补 fresh-write prune；④ entity 三连（空 new_state fail-closed、第 5 注入点 16K 截断、`advance_applied` sidecar 补偿）；⑤ run_context 指纹补 model+review_tier；⑥ 预算账本按 model 前缀逐 record 计价 + retry_error 逐 attempt 入账 + dirty_lines；⑦ deepseek CJK token 估算只减不增 + 已知前缀 64K 动态 cap；⑧ preflight 守门补 tier/timeout/context/anchor；技债补 foreshadowing `origin` 与 chapter_status 五分类单一真源。收官审查直修 8 项并追加双视角 P1：reaper argv[1:] 尾部比对、readiness tier 意图、SUPPLEMENT 补偿、recency 守门、缺 planted_chapter fail-closed、负 timeout、dirty JSON 行、Web/auto-pipeline 写锁覆盖、错误日志 key/prompt 脱敏、cmd_stop 收割落盘、resume tier override、run-all/auto-pipeline/review 全链路写锁。canonical **1622 tests OK**（.venv，需允许本地 127.0.0.1 测试端口；裸沙箱会拦 `test_novel_client`），`verify.sh` exit 0，mock preflight ok，真实配置态 preflight warn/无 FATAL。真模型 capstone 仍未跑（铁律⑥），小说主链路进入“可授权实跑”状态；后续路线转向 iter079-083 drama 模块批次，capstone 可作为独立授权任务插队。
