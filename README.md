...
- **本地 Web**：四阶段工作台、设定/大纲/细纲/正文编辑、任务恢复、全文搜索、版本比较与创作数据；浅色语义变量、中文文案层和共享组件已贯通，Phase A-E 的公共页、单章主流程及高级/辅助页均完成生产级重构。
...
Web 可管理小说与短剧工作区，显示准备情况、任务、草稿、检查结果、章节历史和创作数据。它基于标准库 `http.server` 与原生 JS，默认只绑定本机；当前定位是个人研究工具，不是公网多租户服务。iter153-156 已依次完成 Phase A/B 共享层、Phase C 公共页、Phase D 单章主流程及 Phase E 高级/辅助页，小说 Web UI/UX Phase A-E 已全部落地。
...
最近一次更新：**iter 164**（2026-08-04，收官）。当前状态以 [`docs/AGENT_HANDOFF.md`](docs/AGENT_HANDOFF.md) 为准。短剧 Web Phase A-D 与小说 Web Phase A-E 继续保持；任务卡现在只从服务端验证的 workspace/episode 结果上下文导航，detail/cancel 只恢复 URL 指定 workspace，protected mutation 在读 body 前拒绝歧义分帧。视频 calibration 以 reconciliation 后的 effective 状态计账，同时保留 raw source 状态且不改 ledger/receipt。iter143 在没有绑定该次提交的权威新证据前仍为 unknown，禁止据任务列表缺项重提。总工程验收为 `mock-functional`，浏览器证据为 `local-e2e`，均不等于 `provider-validated`。
...
| D. 逐镜视频 | 每镜 I2V/R2V 输入计划、submit→poll→download、候选选择与跨镜连续性 | 🟨 | **D1-D5 本地闭环已完成**；iter163 将 create 结果封闭为确定未发送/明确拒绝/真正 unknown，并以 append-only CAS receipt 对账历史 unknown；iter164 让 calibration、异常持久化、poll-only 失败与 evidence 共用 effective accounting，raw source 状态继续保留且不改 ledger/receipt。iter142 已验证旧 Episode 1 固定 5 秒样本；iter143 的独立 20 秒样本仍为 unknown，无 task/MP4 且禁止重提，任务列表未增加不是未提交证据。两者都不是 D1-D5 逐镜 adapter；真实逐镜 provider/network、Web submit/poll/cancel、长时主观质量、完整单集与 episode 2+ 媒体仍未验证 |
...
| J. 真 provider 校准与 capstone | 真文本、真图片、真语音、真视频分别 preflight、最小 smoke、单镜、单集、多集校准 | ⏳ 🔒 | 后续严格逐项授权：①只读查询 iter143 task/billing/rejection，upload/create=0；②实现真实 TTS adapter 后授权 1 条语音；③新 namespace 授权 1 个单镜图片/视频样本；④通过后再授权完整单集；⑤episode 2+ 与多集另行授权。任何前序授权都不传递。iter142 的 5.042 秒窄样本与既有真文本/图证据不外推上述阶段 |
...
| 小说续写 Web UI/UX 重构依据 | [`docs/product/NOVEL_WEB_UIUX_REDESIGN_SPEC.md`](docs/product/NOVEL_WEB_UIUX_REDESIGN_SPEC.md) |
