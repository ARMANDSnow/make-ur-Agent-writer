...
- **短剧**：五站 job、分镜 grid、角色库、review/assembly、连续多集、单集四导出、整季母包/阶段快照、逐镜图片 C1-C3、逐镜视频 D1-D4 与 E1 voice/utterance 纯本地契约，以及角色生图和 episode 1 视频安全入口。
...
| 短剧媒体连续性与声音计划 | 119-120 | 🟨 D4 production compose gate 与 E1 显式 Voice Profile/逐句 AudioManifest 已完成；TTS、时间线和本地成片待 121-123 |

历史里程碑见 [`docs/PROJECT_HISTORY.md`](docs/PROJECT_HISTORY.md)，逐轮验收见 [`docs/iterations/README.md`](docs/iterations/README.md)。

## 流水线 SOP（实时状态）

最近一次更新：**iter 120**（2026-07-17，收官）。当前状态以 [`docs/AGENT_HANDOFF.md`](docs/AGENT_HANDOFF.md) 为准。现有短剧 Web 主链已经过桌面与 390px 移动端真人用户路径验证；逐镜图片 C1-C3、逐镜视频 D1-D4 与声音 E1 形成纯本地契约。E1 以显式 Voice Profile/Assignment 将有序 spoken segments 投影为逐句 AudioManifest，缺 voice 或 scope/frozen-cast 不匹配会 blocked，旁白不借角色声音；当前仅有 bounded mock WAV，不连接真实 TTS。指定 provider/model 的真文本 5 calls 与真图 2 张仍是 iter118 的局部校准；真视频/真语音未执行，E2-I 尚未闭环。完整目标流程来自独立的 [A-J 阶段计划](docs/iterations/stage_plan_drama_full_production_pipeline.md)，不是已完成能力清单。总验收为 `mock-functional`，fake-provider 组件为 `local-e2e`；真实证据不代表完整模块或视频 `provider-validated`。
...
| E. 声音与唯一时间线 | 角色 voice、逐句 TTS、旁白、字幕、BGM/SFX 与 `TimelineManifest` | 🟨 | **E1 已完成**：versioned VoiceProfile/Assignment、完整 AudioManifest、显式 speaker/frozen-cast/scope 门禁与 bounded mock WAV；E2 provider-neutral TTS attempt、真实音频时长及 E3 唯一时间线/字幕未实现 |
...
| J. 真 provider 校准与 capstone | 真文本、真图片、真语音、真视频分别 preflight、最小 smoke、单镜、单集、多集校准 | ⏳ 🔒 | iter118 已完成指定 synthetic 样本的五站真文本（5 calls）与 2 张角色真图局部校准；真视频、全角色/多题材、单镜/单集/多集和 SLA 仍未验证，完整 capstone 依赖 B-F |
