...
| 阶段 19（iter 080-081）| **短剧站③分镜 + 站④角色/角色库/AI 绘画骨架**：iter080 解锁 `drama_storyboard` schema/runner/prompt/config、5 赛道原创 storyboard fixtures、Web storyboard API 与可编辑 grid（排序/重生/高光校验）；iter081 新增 `DramaCharacter` / `CharacterSheet` / `ReferenceImage`、`character_designer`、5 赛道原创角色 fixtures、`/w/{name}/characters` 角色库页、写作页站④角色卡编辑/锁定/保存/重画、placeholder SVG 零网络绘图、真实绘图安全骨架（公网校验、PNG/JPEG/WebP、5MB cap），settings 接入 `DRAMA_MODEL` / `AI_DRAW_*`。canonical **1685 tests OK**，`verify.sh` exit 0，mock preflight ok；真模型/真绘图 smoke 未跑（铁律⑥） | 完成 |
...
一条续写指令从输入到输出经过 9 个阶段，下面是各节点当前的打通状态。这是一份活文档，每轮 iter 收官时同步。最近一次更新：**iter 081**（2026-07-09，收官）——**短剧站④角色 + 角色库 + AI 绘画骨架**：短剧模块从站①核心设定、站②钩子、站③分镜推进到角色设定表闭环；新增 `DramaCharacter` / `CharacterSheet` schema、`character_designer` runner、5 赛道原创角色 fixtures、角色库页 `/w/{name}/characters` 与写作页站④角色卡编辑/锁定/保存/重画，默认零网络生成 placeholder SVG，真实绘图 client 仅做安全 wiring（公网 endpoint 校验、PNG/JPEG/WebP、5MB cap、key 不落盘）。canonical **1685 tests OK**（.venv），`verify.sh` exit 0，mock preflight ok，真实配置态 preflight warn/无 FATAL；真模型/真绘图 smoke 未跑（铁律⑥）。上一轮 **iter 080**（2026-07-08，收官）——**短剧站③分镜生成 + Grid 编辑器**：短剧模块从站①核心设定、站②钩子推进到站③分镜表闭环；新增 schema/runner/prompt/config、5 赛道原创 fixtures、同步 200 API 与 Web grid 编辑器，支持重生本镜、排序、服务端重排 `shot_no`、双高光硬拒与零高光 soft warning。再上一轮 **iter 079**（2026-07-06，收官）——**Capstone 前写锁与预算硬化（P2）**：Web 手工写端点统一走 `workspace_reserved` + `workspace_lock` flock，CLI/driver 写作期间 PUT draft/outline/KB/start-point/chapter-plan/entity/relationship/writer-style/premise/drama 保存返回 409 不落盘；`apply-advance` 与 write-book auto-advance 互斥；readiness 补 `entity_proposal_gap:NN` warning；新增 mock 预算演练开关；小说主链路仍处于“可授权实跑”状态。
...
| U.10 | Web drama 4 站审查向导（前 4 站）| ✅ | iter 037-038（drama wizard 5 字段 + `wizard_input.json` + `creation_standard.snapshot.md`；`/w/{name}/write` 4 tab；站 ①/② mock fixture-driven；iter 038 修 hook picker listener leak / rapid-click race）；iter080 解锁站③分镜表：mock fixture/真模型 wiring、storyboard API、grid 编辑/排序/重生/高光校验；iter081 解锁站④角色：角色表 schema/runner/API、角色库页、角色卡编辑/锁定/保存/placeholder 重画，真绘图 wiring 留后续实测 |
