...
最近一次更新：**iter 129**（2026-07-18，收官）。当前状态以 [`docs/AGENT_HANDOFF.md`](docs/AGENT_HANDOFF.md) 为准。短剧 Web 主链已完成两次真人用户路径验证：iter118 覆盖桌面与 390px 移动端，iter124 再以 1440×900 fresh 两集 SOP 验证多集内容、编辑导航、角色真图、交付与任务历史。A1/A2 RenderPlan、visual-only override 与精确 stale matrix，B1 跨集资产 Used-By/停用治理、B2 ArtDirection 多 scope 解析冻结、B3 资产管理 Web，逐镜图片 C1-C3、逐镜视频 D1-D4、声音/时间线 E1-E3，以及 F1/F2 本地合成/可编辑导出已形成纯本地链；iter125-129 的 scope-specific 真文本只构成局部协议校准，真视频仍未执行，TTS 未测试。完整目标流程来自独立的 [A-J 阶段计划](docs/iterations/stage_plan_drama_full_production_pipeline.md)，不是已完成能力清单。总验收为 `mock-functional`，mandatory local-drama 组件为 `local-e2e`。
...
| J. 真 provider 校准与 capstone | 真文本、真图片、真语音、真视频分别 preflight、最小 smoke、单镜、单集、多集校准 | ⏳ 🔒 | iter118 与 iter124 各完成一次五站真文本 + 2 张角色真图局部校准，iter123 有 synthetic 最小校准，iter125-129 各完成一次文本协议校准；真语音/真视频、全角色/多题材、单镜/单集/多集和 SLA 仍未验证 |
