...
| Agent 记忆与历史归档 | 093 | ✅ 默认必读链路瘦身，当前/历史职责分离 |

历史里程碑见 [`docs/PROJECT_HISTORY.md`](docs/PROJECT_HISTORY.md)，逐轮验收见 [`docs/iterations/README.md`](docs/iterations/README.md)。

## 流水线 SOP（实时状态）

最近一次更新：**iter 093**（2026-07-13，收官）。本轮不改业务行为：默认新 session 只读 `AGENTS.md` + handoff，历史 stage/iteration 按需加载；原必读链约 2907 行缩为 179 行。最新业务行为基线仍是 iter 092 的短剧多模态可恢复编排，真文本、真生图和真视频仍需分别授权。
...
| Web/短剧 | 可编辑工作台；短剧五站、导出、图片/视频 job | ⚠️ | 本地 mock 闭环；真多模态待分段授权 |
