...
| 阶段 19（iter 080）| **短剧站③分镜生成 + Grid 编辑器**：新增 `drama_storyboard` schema/runner/prompt/config，5 赛道原创 storyboard fixtures，Web API `GET/POST/PUT /drama/storyboard` + `rewrite-shot`，站③ tab 解锁为可编辑 grid（景别/运镜/时长/画面/旁白/台词/高光/排序/重生/保存），站②未完成与 novel workspace 均 400，写入走 `_workspace_write_guard` + `write_json`，双高光硬拒、零高光 soft warning。canonical **1661 tests OK**，`verify.sh` exit 0，mock preflight ok；真模型 smoke 未跑（铁律⑥） | 完成 |
...
一条续写指令从输入到输出经过 9 个阶段，下面是各节点当前的打通状态。这是一份活文档，每轮 iter 收官时同步。最近一次更新：**iter 080**（2026-07-08，收官）——**短剧站③分镜生成 + Grid 编辑器**：短剧模块从站①核心设定、站②钩子推进到站③分镜表闭环；新增 schema/runner/prompt/config、5 赛道原创 fixtures、同步 200 API 与 Web grid 编辑器，支持重生本镜、排序、服务端重排 `shot_no`、双高光硬拒与零高光 soft warning。canonical **1661 tests OK**（.venv），`verify.sh` exit 0，mock preflight ok，真实配置态 preflight warn/无 FATAL；真模型 smoke 未跑（铁律⑥）。上一轮 **iter 079**（2026-07-06，收官）——**Capstone 前写锁与预算硬化（P2）**：确认当前无“已发现但未修”的 P1/High 后，本轮补齐 capstone 前最贴近生产长跑风险的边界：Web 手工写端点统一走 `workspace_reserved` + `workspace_lock` flock，CLI/driver 写作期间 PUT draft/outline/KB/start-point/chapter-plan/entity/relationship/writer-style/premise/drama 保存返回 409 不落盘，writer-style/extract 上传暂存与 worker 写卡也补 flock；`apply-advance` 与 write-book auto-advance 互斥并在锁冲突 exit 4；readiness 对 approved/caveat 章缺 proposal+sidecar 显式 `entity_proposal_gap:NN` warning，收尾复核补 `resume_from-1` 前章同口径；新增 `MOCK_COST_CNY_PER_1K_TOKENS` 正有限值演练开关，默认 mock 仍 0 成本；审查修复 Web lock holder 409 与 Web job JSON 原始诊断泄漏，只回 `source/started_at` 摘要。小说主链路仍处于“可授权实跑”状态。再上一轮 **iter 078**（2026-07-05，收官）——六方审查 P1 修复批（8 组）+ 技债 2 项，完成 reviewer 分数护栏、workspace 写锁主链路、rolling 落盘顺序、entity 三连、run_context 指纹、预算账本、CJK token/cap、preflight 守门、foreshadowing origin 与 chapter_status 五分类下沉。
...
| U.10 | Web drama 4 站审查向导（前 3 站）| ✅ | iter 037-038（drama wizard 5 字段 + `wizard_input.json` + `creation_standard.snapshot.md`；`/w/{name}/write` 4 tab；站 ①/② mock fixture-driven；iter 038 修 hook picker listener leak / rapid-click race）；iter080 解锁站③分镜表：mock fixture/真模型 wiring、storyboard API、grid 编辑/排序/重生/高光校验；站④角色仍待后续开放 |
