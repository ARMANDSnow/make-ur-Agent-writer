...
- 开发默认 mock。1738 个单测本地跑完，不烧 token；`tests/__init__.py` 里强制 `OPENAI_MODEL=mock`，避免 `.env` 漏进测试。
...
| `style-fingerprint build-baseline` / `style-fingerprint inspect-draft --chapter N` | 量化文风 baseline v2 与草稿指标检查（iter 083/086，默认 mock-only、仅存聚合统计量；v1 需重建） |
| `style-drift --chapter N` / `style-drift-report --limit N` | 对草稿做文风漂移检测、写入已有 meta、按最近章节窗口报告 severity（iter 084/086；手工写 meta 受 workspace 锁保护） |
...
一条续写指令从输入到输出经过 9 个阶段，下面是各节点当前的打通状态。这是一份活文档，每轮 iter 收官时同步。最近一次更新：**iter 090**（2026-07-12，收官）——**短剧五站真模型 Job 化 + 全链 Smoke**：站①-⑤统一为 202/job/poll/cancel，站①②补齐严格 schema 与真/mock wiring；真文本需每请求确认 + finite 正预算 + 有界超时，并在写 artifact 前结算成本。Web 已补刷新恢复、候选持久化、任意 episode 角色 job 重连与五站 fail-closed/stale/rollback。mock 全链五 job、四导出、Insights 成功，零 LLM/零视频。本轮未授权真文本；已授权的单次真生图在 120s 超时后未重试。三视角修后复核 PASS；canonical **1857 tests OK**，`verify.sh` exit 0，mock preflight ok，真实配置 preflight warn/无 FATAL。

上一轮 **iter 089**（2026-07-12，收官）——**短剧真实生图接入 + 视频 API 安全预备**：站④重画已支持显式 `AI_DRAW_MODEL=gpt-image-2` opt-in，并可整对复用既有 OPENAI base/key；Web 保存当前角色表后以 JSON 逐请求确认发起，图片经 HTTPS、结果域 allowlist、5MB/base64/magic/MIME 与原子落盘守门，requested/provider/actual 元数据诚实留痕。素材/视频任务客户端已就绪，但真实生成仍需 `allow_real_video=True`，本轮没有调用视频。原创真图 smoke 返回 `gpt-image-2-codex / auto / 940×1673 PNG`。correctness、security/boundary、Web/integration 三视角 findings 全修并复核 PASS；canonical **1839 tests OK**，`verify.sh` exit 0，mock preflight ok，真实配置 preflight warn/无 FATAL。

再上一轮 **iter 088**（2026-07-11，收官）——**短剧四格式导出 + Insights + 第 2 集重生**：`episode_NN.json` 作为唯一真源，可安全下载 JSON/Markdown/CSV/Comfy 模板；短剧 Insights 聚合有界成本、时长达标率和钩子分布。episode 参数化已贯穿四站与 Web，第 2 集通过完整第 1 集受控初始化，继承设定、排除已用钩子并注入季角色 visual signature；无新角色可跳过站④，有新角色则强制补齐角色表。correctness、security/boundary、Web/integration 三视角 findings 全修并复核 PASS。canonical **1813 tests OK**，`verify.sh` exit 0，mock preflight ok，真实配置 preflight warn/无 FATAL，Python/Node/diff 通过；真模型、真绘图、真实 ComfyUI smoke 未跑。
...
| U.10 | Web drama 4 站审查向导（前 4 站）| ✅ | iter 037-038（drama wizard 5 字段 + `wizard_input.json` + `creation_standard.snapshot.md`；`/w/{name}/write` 4 tab；站 ①/② mock fixture-driven；iter 038 修 hook picker listener leak / rapid-click race）；iter080 解锁站③分镜表：mock fixture/真模型 wiring、storyboard API、grid 编辑/排序/重生/高光校验；iter081 解锁站④角色：角色表 schema/runner/API、角色库页、角色卡编辑/锁定/保存/placeholder 重画，真绘图 wiring 留后续实测；iter082 解锁 drama_reviewer + episode assembly + episodes/detail 复核页，支持站④后一键评审并组装、input fingerprint stale、suggestion apply；iter088 补齐四格式安全导出、drama Insights 与有界第 2 集重生（继承/去重 hook/季角色 signature/角色站跳过或强制补齐） |
...
| U.15 | Web 文风 drift 只读展示 | ✅ | iter 084 章节详情顶部文风 badge + “文风”tab；iter086 修 `#style` 刷新、全 tab 失败态、null/坏数组降级、未检测/已跳过区分、Advisor 来源与共享 tabs ARIA/键盘导航；不展示样本或额外原文片段 |
