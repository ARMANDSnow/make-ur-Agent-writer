...
- Iteration 019: fuller `write_book.sh` automation, chapter failure resume/retry, and optional plan-aware entity advance workflow. First real-model run on the English workspace (workspace3) belongs here.
- Iteration 020+: hosted-product layer — web UI / SaaS / per-user workspace isolation. Also: Japanese / Korean splitter heuristics, and translating agent prompt templates from Chinese.
