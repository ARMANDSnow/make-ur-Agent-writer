...
| 短剧 | 五站 job、分镜 grid、角色库、review/assembly、连续多集（计划上限 100）、单集四导出、严格整季母包/阶段快照、Insights、episode 1 视频 job、多模态可恢复编排；真跑前已收紧五站/媒体 durable attempt、产物血统、provider/账号与实际 peer，并解析复核 MP4 时长/尺寸/比例 | 真文本/全角色真生图/单次真视频需分别授权实测；真实质量仍需人工判定；episode 2+ 视频、真 ComfyUI 与更广 codec 支持未做 |
| 集成 | Aeloon 插件/MCP 双轨已实现；详情见 [`AELOON_INTEGRATION.md`](AELOON_INTEGRATION.md) | Aeloon vendored 基线与主仓后续版本需按集成文档同步 |

## Latest Accepted Evidence

- 站②真模型 prompt 注入有界站①设定视图；钩子 schema 本地固定“情绪/悬念/反差”三类与顺序，不能由模型任意改型。
- 五站 Web 真文本 attempt ledger 严格拒绝损坏/符号链接，绑定输入/provider/账号与 canonical 产物；付费响应已提交但 callback/终态账本未完成时可零网络采纳，无法证明才要求核账重试。
- 图片冻结 provider、cast 与渲染输入；确定未发送不占 attempt/预算，`artifact_received` 后本地写盘失败保持可恢复，恢复路径被限制为 workspace 内普通文件。
- 带凭据图片/视频 API 统一在 TLS connect 后校验实际 socket peer，再发送 path/body/Authorization；redirect 与超限响应继续 fail-closed。
- 视频 ledger 冻结授权预算/估价/timeout；`RequestNotSentError` 不消耗单次提交，其他提交异常保持 unknown；回读和状态会重解析 MP4 时长、尺寸、比例与 sample table。
- 校准报告交叉读取 durable text/video ledger，并把 image pre-request marker 单列为 unknown，避免 crash-window 报告 0 exposure。
- canonical **1987 tests OK**；`verify.sh` exit 0；mock preflight **0 FATAL / 0 WARN**；294 项短剧聚焦回归、Python/shell 语法与 `git diff --check` 通过。未运行真文本、真生图或真视频。
- 6 个调研 subagent + correctness、security/boundary、billing/resume 3 个收官只读 subagent 完成复核；确认 findings 已修复，无未修 P0/P1。
...
- Iter 092 的多模态 state machine 支持 fresh/resume、独立授权、预算/deadline 与生图重试；Iter 094 补齐校准证据；Iter 097-098 继续收口旧旁路、provider 身份、五站/媒体 crash window、产物血统与实际媒体规格。
- 文本证据按五站记录 task/model SHA、调用数、耗时与 cost，并以严格 attempt ledger 绑定 endpoint/账号/输入/产物；成功产物可零网络恢复，未知提交才要求对账。
- 图片证据按角色/attempt 记录 objective metadata、耗时、尺寸和 SHA；视频以 durable ledger 区分确定未发送、提交未知、已提交、终态失败与成功，报告交叉重读 ledger 和本地产物，避免 crash window 低报付费请求。
...
4. **短剧媒体**：真 ComfyUI workflow、episode 2+ 视频、更广 codec/容器支持及真实多模态质量仍未验证；当前 MP4 有界解析仍需真实供应商样本校准。
5. **集成同步**：Aeloon 内置副本不是自动跟随主仓，需要按集成文档明确同步。
6. **多集查询性能**：100 集时 `GET /drama/episodes` 会在状态与季包 readiness 间重复读取部分文件，可后续缓存一次请求内的扫描结果。

## Next Candidates

- 低风险工程轮：provider 幂等键/资产上传恢复调研、100 集只读扫描优化或已登记 P2 技债。
...
iter 098 以 6 个调研视角和 3 个收官审查视角继续审计“真文本 → 全角色真生图 → 单次真视频”。主线补齐站①→②上下文、钩子 schema、五站 canonical 产物血统与 Web 付费 attempt 恢复；图片/视频请求在敏感数据发送前校验实际 peer，图片 artifact 可零网络收尾，视频区分确定未发送/提交未知并重解析 MP4 规格；校准报告交叉 durable ledger。最终 1987 项 canonical、`verify.sh` 与 0 WARN/FATAL mock preflight 全绿，未运行真文本、真生图或真视频。
