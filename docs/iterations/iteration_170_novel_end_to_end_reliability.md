...
- A170-04：F06 usage 缺失/坏值阻断下一请求，真实链累计计量（初始硬预算目标，后续授权变更与provider限制见结果）。
...
- A170-08：PASS。新增实现 `5882a33` 上 canonical schema v3 / `canonical-novel-mock-offline` 通过；1981 tests / 15 steps / 96秒，run `48d32684f8c0482c8b2e7c2903daba63`，`tracked_scope_clean=true`，仅 `mock-functional`。此证据替代此前工程验证。
- A170-09：干净工作区真实准备、规划、续写与长流程/恢复实测，逐步记录模型调用、费用与真实结果；必须获得 provider-validated，未通过不得宣称本轮完成。

## Implementation Notes

F25：补外审18次成功、正文不变，但第三章4票/7.46分被正确拒绝；实际Web重试提示与资格不一致。新增完整同版本、无硬拒绝external_review_reject的单章确认归档重生成，严格匹配同章同原因blocked终态；GET/POST及worker共享资格、指纹和claim，不开放普通force。81项聚焦通过，correctness与security/boundary独立审查完成；新增坏类型halt原因曾导致TypeError，已修为blocked并增量复审通过；不将重生成宣称为按旧反馈定向修订。5882a33 canonical 1981项/15步骤/96秒通过；真实浏览器确认仅恢复第3章、100请求/45分钟，ceff8cd2abd74dc7ac51e21ef38f6626已成功完成，第三章内审5票/7.78分，完整外审与strict均通过；旧稿按原SHA归档、前两章6个文件hash不变。

F24（P1）：a69f2a29真实45分钟截止于第三章外审顾问，内审同hash报告竟让严格状态提前批准。仅review_target完整结束后写精确bool外审完成凭证；strict及meta同步要求凭证，旧报告保守补审而不迁移。补审分类同时阻断failure/panel_halted及主审Approve但需人工/硬拒绝的盘面，防止回落自动重写。337项相关聚焦与两路独立复审通过，含第五票后顾问取消、标记落盘失败、内审覆盖清标、补审writer零调用和advance补偿。真实三章正文及rolling均完整，但新规则下均仅external_review_incomplete，随后真实18次补审、零WRITE且三章正文不变：前两章严格通过，第三章4票/7.46分明确Reject并blocked；三份外审完成凭证均存在，不沿用旧的提前通过投影。
...
F22：标准档首次14–15次模型调用、一次重写增加7次，发现按章20的默认被错误用作不可提高的最大值。主写作与显式失败章恢复最大值改为160，默认仍按章20/恢复20，保留其他计费/时限/资格/确认边界。41项聚焦、两路只读审查与增量复审通过；浏览器3章手填100后改1章仍100，提示可调上限160。当时正在运行的60次额度任务未更改；后续新任务显式100次。
...
F19：多章正常写作至少约15请求/章，Web却固定20次硬上限。先记录设计缺陷，改为已验证章节数×20并保留全局160上限，两个表单按章数同步未手改默认值；手动低值保留，当时单章恢复仍20；F22将默认20与最大160分离。132项聚焦通过；两路审查发现预设程序改章数漏触发同步，已补input派发和联合Node回归，增量复审通过。
...
- A170-04：工程PASS，Web/CLI缺失usage、坏usage转换和日志sink失败阻断下一请求；真实外层保留累计预留且不回收未知费用；provider忽略输出上限，不能证明严格费用上界。用户随后明确暂不考虑预算继续测试，usage按用户单价估算，不是权威账单。
...
- A170-08：PASS。新增实现 `5882a33` 上 canonical schema v3 / `canonical-novel-mock-offline` 通过；1981 tests / 15 steps / 96秒，run `48d32684f8c0482c8b2e7c2903daba63`，`tracked_scope_clean=true`，仅 `mock-functional`。此证据替代此前工程验证。
- A170-09：PASS（有界三章与恢复，`provider-validated`）。干净工作区完成第三部末尾起点的10章知识提取、准备、36次辩论发言与完整投票、大纲和3章细纲；3章正文均通过严格内外审，外审完成凭证、正文hash、当前计划/起点与滚动记忆一致。恢复任务`ceff8cd2abd74dc7ac51e21ef38f6626`完成；实际Web再次提交完成范围，任务`53b62a0c30b442aab6f4472a1bc3b580`成功，零新增模型请求（请求243→243、LLM rows247→247）且9个正文/meta/review文件不变。 F24补外审18次均成功且零WRITE；第3章软拒稿后F25真实确认归档重生成，前两章6文件不变、旧第三章正文完整保存。10–20章不在通过结论内。
...
- `reason`: 合集章名不能全局去重；日志压缩必须保持逻辑历史与当前文件身份，缺计费数据的停止规则须覆盖CLI；默认请求额度与最大值分离，写手与评审共享当前计划，完成态须有外审阶段凭证，受控恢复绑定同章同原因终态。
...
| `src/openai_stream.py`, `config.py`, `cost_estimator.py` | 可取消有界流式、原生usage与未知费用保护 |
| `src/debater.py`, `plot_planner.py` | 完整辩论有界摘要、冻结问题与缺票恢复、计划覆盖验证 |
| `src/partial_recovery.py`, `reviewer.py`, `chapter_status.py` | 完整失败稿恢复、评审当前计划、外审完成凭证 |
| `src/web/write_recovery.py` | 同章同原因终态的明确外审拒稿受控恢复 |
| `scripts/write_book.sh` | tier透传 |
| `tests/test_iter170_reliability.py`及旧编辑夹具 | 新反例与版本协议兼容回归 |
| partial recovery / review plan / external completion / Web recovery 等专项测试 | 后半轮恢复和阶段完成反例 |
| 本文、索引、审计报告 | 方案和阶段证据 |
...
最终收官（2026-09-06）：干净工作区完成第三部末尾起点的10章知识提取、准备、36次辩论发言与完整投票、大纲和3章细纲；3章正文均通过严格内外审，外审完成凭证、正文hash、当前计划/起点与滚动记忆一致。恢复任务`ceff8cd2abd74dc7ac51e21ef38f6626`完成；实际Web再次提交完成范围，任务`53b62a0c30b442aab6f4472a1bc3b580`成功，零新增模型请求（请求243→243、LLM rows247→247）且9个正文/meta/review文件不变。 本地安全证据在系统临时测试根，仓库文档仅保留元数据；实现5882a33，后续仅docs-only收官。

最终脱敏复核证据：`/private/tmp/novel-audit-20260905/real/final-resume-safe.json`、`final-before-resume-safe.json`、`final-after-resume-safe.json`、`real-validation-safe.json`与`f25-archive-safe.json`。已完成Luna响应按用户单价暂估约0.295662元；包含历史unknown的累计预留6.592397元均非权威账单，旧unknown未重提。测试Web仍在127.0.0.1:8768运行，当前无活动任务。
