...
- **前端 E2E（preview MCP，mock，web-mock 端口 8765）**：
  ...
  - 未能实景验证项（诚实记录）：drama 徽章 🎬+边框（本机无 drama workspace，由静态 JS/CSS 测试覆盖）；空书架文案（本机有 3 个 workspace 非空，由 `assertNotIn("epub")` 测试覆盖）；cancel-and-leave 端到端（需真实 running job，模态渲染 + cancel 调用已分别静态验证）。
- **代码审查（铁律⑨）**：
