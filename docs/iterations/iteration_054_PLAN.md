...
## Acceptance（mock 段已收官 2026-06-13；真模型段待授权）
### mock 段（门槛）✅ 全部达成，verify.sh exit 0、977→986 单测 OK、auto-pipeline 跑通
- 全量回归零失败：965 基线 → **986**（+21 本轮新增 mock 测）；每提交独立 verify.sh 绿。
- 钉死项（逐项 mock 测 + 提交号）：
  - ✅ `_normalized_context` 起点 clamp（有/无起点）+ `before_start_line_limit` 四态 + apply 端 skip/clamp — `058fb9c`（054a-2/3）
  - ✅ source_excerpts 三路过滤（select_for_chapter choke-point）— `c67f517`（054a-1）
  - ✅ style_examples 起点拒切（apply 端守卫）— `058fb9c`
  - ✅ 关系 timeline chapter_id 强制 + `_load_extractions` start-aware（有/无起点）— `857fec8`（054b 核心 + 折叠 054a-4）
  - ✅ entity_graph 起点 stale sidecar（matcher 三态 + bootstrap skip/rebuild + apply 盖章）— `230a03a`（054b 自动化）
  - ✅ 覆盖闸前移（plan-chapters 硬 blocker + set-start-point 即时报）— `3c860c5`
  - ✅ `rebuild-for-start` 编排（无起点拒 + 全链 + 窗口收窄 + 双 sidecar 盖章 + --no-apply）— `c823e87`
  - ✅ ingest-to-start（单卷截断 + 多卷删卷 + 下游零起点后内容）— `7cb11b6`（054d 主线机制）
  - ⏳ A-M5 逃生门透传 / M3 `approved_with_block_issues` 标记 — **顺延 054c 搭车修**（happy path 不触发，非 mock 门槛）
- 既存红门禁修复：auto-pipeline debate 步透传 force — `622f0c3`（与 054a 无关，git stash 已证）。

### 真模型段（深起点 ≤¥20，2026-06-13 授权实跑，部分收官 / 续写改日续跑）
用户授权 `CONFIRM_REAL_MODEL_SMOKE`，深起点拍板 **`longzu_2_ch001`（龙族II 开篇，idx12，卷边界；起点前 12 章 / 起点后 98 章真实素材都物理在场——彻底打破 ch024 结局章+70k cap 截断的旧巧合）**。两个同源克隆从 longzu `小说txt` 构建：`longzu_054c_ingest`（ingest-to-start 物理截断）/ `longzu_054c_full`（全量+设起点+054a/b 过滤）。

**✅ diff oracle（核心泄露验收，已机器证毕、零真模型成本）**：
- `_normalized_context`（style_examples + source_excerpts 的 bootstrap 采样源——最严重 verbatim 口）在 ingest 截断模式 vs full+clamp 模式 **逐字节一致**（各 123141 bytes，`diff` 空）。
- 采样源只覆盖 longzu_1/longzu_2，起点后专属标志（黑月之潮/奥丁之渊）两模式皆 **0**。
- ingest 截断实证：kept 13 章 / dropped 97 / longzu_2.txt 截断 + 后 5 卷 normalized 文件删除。
- → full+filter ≡ 物理截断，过滤正确性的机器证明。叠加 986 单测对消费/构建端过滤的覆盖，**封口正确性已闭环**。

**⏳ 续写 ch1-3 + 正文泄露体检 — 改日续跑（中转站拥堵，非阻断性）**：
- 实跑遇 aetherheartpool 源站 Cloudflare Tunnel `Error 1033/530` 宕机（provider 侧，已恢复但恢复后拥堵）；longzu 章节巨大（20-30K 字）高档 extract 极慢（分块章实测 967s/章）。窗口提取 2/10 后用户拍板**收于确定性 oracle**：核心封口已证，续写冒烟改日拥堵缓解再跑。
- 真模型段累计成本 **¥2.93**（多为宕机期 530 失败重试损耗），远低于 ¥20。
- **续跑现成状态**：克隆 `longzu_054c_full`/`longzu_054c_ingest` 保留（workspaces gitignore，不污染 git），已存 ch004/ch005 提取 + 完整 ingest 截断底座；改日用 `/tmp/extract_window.py`（健壮驱动：流式+每章超时重试+断点续跑+绕分块 monkeypatch）补完窗口 → `rebuild-for-start` 收尾 → drive-book 续写 ch1-3。
- 写作质量（次要）：low 档 Approve（≥2/3）+ panel ≥6.5 —— 随续写一并补。

**真模型段运维发现（沉淀，非泄露问题）**：① longzu 章节 20-30K 字，分块章（>24000）走 3 子调用、拥堵下极慢（967s）；绕分块（单调用，30K 仍在 128K context 内）快得多。② aetherheartpool 偶发挂请求/Tunnel 宕机，且 **LLM 调用无 per-call 超时** → 挂起永久阻塞；实跑须用超时+重试驱动。③ **`extract_all` 静默吞每章异常进 failure 记录、不向编排层抛**（extractor.py:393）——宕机时整个 rebuild 表现为"0 提取、无报错"，编排层无感；幸有 054b 覆盖闸（plan-chapters blocker）兜底拦"底座没建好"。已立背景任务另案修。
