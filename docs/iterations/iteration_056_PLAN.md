...
- **V5 续写欠账 ✅ ch1-3 完整 Approve（2026-06-18 搭车跑，彻底了结 iter054 欠账）**：`set-start-point longzu_2_ch001` → `rebuild-for-start --no-chunk`（8 章 extract + KB 重建，`--no-chunk` 单调用提取验证 iter055 加固）→ `drive-book` debate/plan/write。**ch1 Approve 5/5 panel 7.82 / ch2 Approve 3/5 panel 7.36 / ch3 Approve 5/5 panel 7.52**，三章共 17173 字、全部零污染、零指令泄露（接续起点"卡塞尔学院→宿舍调查→训练馆"连贯剧情）。真实总成本 **¥7.54**（远低 ¥15）。续写书**不注入风格卡**（仅 premise），M2 自检确认正文无风格卡指令——顺验"仅 premise"边界在续写侧正确。
  ...
  - **ch2/3 一度受阻 → 直连修复后跑通（关键根因）**：ch2/3 首跑连续 3 次 resume 均在写 ch2 撞 `InternalServerError / Connection error`，起初疑为中转站服务端抖动；后实测发现**真因是 Clash 系统代理路径不通**（`curl -x 127.0.0.1:7897` → HTTP 000；直连 `--noproxy` → 401 可达）。`with_proxy.sh` direct 分支显式 `export NO_PROXY` 钉死 aetherheartpool.top 直连（commit `4cdd9bd`）后 resume，ch2/3 一次跑通。中转站恢复后仍偏慢（直连 ping 152s），但绕开不通的代理即可完成。断点续跑全程 ch1 approved 正确 skip、零重复花费。

**真模型结论**：风格卡核心（提取 V1 + 端到端真写 V3）真模型验证通过、并修复了一个 mock 测不出的 schema 缺陷；V5 续写欠账 **ch1-3 完整 Approve 彻底了结 iter054 欠账**（panel 7.82/7.36/7.52、共 17173 字、真实 ¥7.54）。两个关键运维发现：① 起点重置须连带清 rolling + 旧 drafts（双重污染）；② aetherheartpool 中转站须直连、走 Clash 代理不通（`NO_PROXY` 钉死）。注入/缓存/gate 由 mock 逐字节 + preview 覆盖。
