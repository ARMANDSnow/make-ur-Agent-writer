...
def render_workspace_continue(name: str, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '<span class="muted">严格续写，自动重写与评审</span></div>'
        '<div class="card-body">'
        ...
        '<div class="form-grid">'
        '<div class="field"><label>本次人民币额度上限</label><input name="budget_cny" type="number" min="0" step="0.1" value="10" placeholder="0 = 不限制；离线模式不消耗真实额度"></div>'
        ...
        '<span></span>'
        '<button type="submit" id="write-book-submit" class="btn btn-paid" data-ui-action="paid">确认并开始续写</button>'
        '</div>'
        '</form>'
...
def render_workspace_reviews(name: str, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '<h1>评审聚合</h1>'
        '<p class="muted">汇总每章结果、分类评分、文字检查与修改建议。</p>'
        '</div>'
        '</header>'
        '<section class="section">'
...
def render_workspace_insights(name: str, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '<h1>创作数据</h1>'
        '<p class="muted">查看每章额度、内容复用情况与评审分数。数据来自已保存的任务记录，不发起新调用。</p>'
        '</div>'
        '</header>'
        '<section class="section">'
