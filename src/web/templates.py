...
def render_workspace_continue(name: str, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '<div class="field"><label for="continue-request-limit">模型请求上限</label><input id="continue-request-limit" name="max_model_requests" type="number" min="1" max="160" value="20" aria-describedby="continue-request-limit-help"><small id="continue-request-limit-help">默认每章 20 次，可调至任务上限 160 次；重写也计入，预算或时限也可能使任务提前停止。</small></div>'
...
def render_workspace_workbench(name: str, workspaces: Iterable[str]) -> str:
    ...
    main = (
        ...
        '<div class="field"><label for="prepare-budget-input">本阶段预算上限（元）</label>'
        '<input id="prepare-budget-input" name="budget_cny" type="number" min="0.1" max="3" step="any" value="3" required>'
        '<span class="muted">额度不足时可调整后重试；开始前会再次确认额度。</span></div>'
        '<div class="form-actions" style="align-items:flex-end">'
        ...
        '<div class="field"><label for="outline-budget-input">本阶段预算上限（元）</label>'
        '<input id="outline-budget-input" name="budget_cny" type="number" min="0.1" max="33" step="any" value="8" required>'
        '<span class="muted">额度不足时可调整后重试；开始前会再次确认额度。</span></div>'
        '<div class="form-actions" style="align-items:flex-end">'
        ...
        '<div class="field"><label for="plan-target-chapters">计划章节数</label>'
        '<input id="plan-target-chapters" name="target_chapters" type="number" min="1" max="200" value="5"></div>'
        '<div class="field"><label for="plan-budget-input">本阶段预算上限（元）</label>'
        '<input id="plan-budget-input" name="budget_cny" type="number" min="0.1" max="10" step="any" value="2" required>'
        '<span class="muted">额度不足时可调整后重试；开始前会再次确认额度。</span></div>'
        '<div class="form-actions" style="align-items:flex-end">'
        ...
        '<input id="write-request-limit" name="max_model_requests" type="number" min="1" max="160" value="20"><small>默认每章 20 次，可调至任务上限 160 次；重写也计入。</small>'
        '</div>'
        '<div class="form-actions" style="align-items:flex-end">'
...
def render_workspace_chapter_detail(name: str, chapter_no: int, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '<textarea id="draft-edit-area" disabled rows="24" placeholder="载入中…"></textarea></div>'
        '<p class="muted" id="draft-edit-help">保存后，本章及后文需要复核；请从最早修改章开始“保存并重新检查”。通过后更新正文摘录记忆，旧人物推进不再生效。</p>'
        '<div class="form-actions">'
        '<button type="button" id="draft-save" disabled class="btn btn-secondary">保存</button>'
        '<button type="button" id="draft-save-review" disabled class="btn btn-paid" data-ui-action="paid">保存并重新检查</button>'
        '</div>'
