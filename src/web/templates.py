...
def render_workspace_episodes(name: str, workspaces: Iterable[str]) -> str:
    main = (
        ...
        '</div>'
        '<div class="cluster">'
        f'<a class="btn btn-secondary" href="/w/{escape(name)}/write?episode=1#characters">回到第 1 集创作台</a>'
        '</div>'
        '</header>'
        '<section class="section">'
...
def render_workspace_workbench(name: str, workspaces: Iterable[str]) -> str:
    ...
    main = (
        ...
        '<p class="muted" style="margin:8px 0">从预置库选一张，或上传样本提取；可在下方编辑器微调。仅 premise 自创书生效，下一章写作时应用。</p>'
        ...
        '<div class="form-actions"><button type="button" id="style-extract-btn" class="btn btn-secondary btn-sm">提取风格卡</button></div>'
