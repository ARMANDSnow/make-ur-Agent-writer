...
def list_episodes(workspace: str) -> List[Dict[str, Any]]:
    ...
    for path in sorted(ep_dir.glob("episode_*.json")):
        if path.name.endswith(".meta.json") or path.name.endswith(".review.json") or path.name.endswith(".setup.json") or path.name.endswith(".storyboard.json"):
            continue
