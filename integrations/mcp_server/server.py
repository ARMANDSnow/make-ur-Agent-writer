...
def _client_from_env() -> NovelClient:
    return NovelClient(
        ...
        request_timeout_s=float(os.environ.get("NOVEL_REQUEST_TIMEOUT_S", "30")),
        poll_interval_s=float(os.environ.get("NOVEL_POLL_INTERVAL_S", "2.0")),
        job_timeout_s=float(os.environ.get("NOVEL_JOB_TIMEOUT_S", "3600")),
    )
...
def _cfg_from_env() -> NovelOpsConfig:
    return NovelOpsConfig(
        ...
        write_budget_cny=float(os.environ.get("NOVEL_WRITE_BUDGET_CNY", "5.0")),
    )
