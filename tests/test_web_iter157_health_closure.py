...
class SafeExceptionProjectionTests(_WorkspaceFixture):
    ...
    def test_drama_overview_exception_degrades_without_unbound_error(self) -> None:
        root = self.workspace("overview")
        (root / "data").mkdir()
        workspace_meta.write("overview", type="drama")
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr), mock.patch(
            "src.web.drama_view.collect_drama_progress",
            side_effect=RuntimeError(_SECRET),
        ):
            overview = routes._workspace_overview("overview")
        self.assertEqual(overview["readiness"]["blockers"], ["drama_progress_error"])
        projected = json.dumps(overview, ensure_ascii=False)
        self.assertNotIn(_SECRET, projected)
        self.assertNotIn(_SECRET, stderr.getvalue())

    def test_wizard_5xx_and_expansion_degrade_are_fixed_and_traceable(self) -> None:
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr), mock.patch.object(
            wizard, "init_workspace", side_effect=OSError(_SECRET)
        ):
        ...
        self.assertRegex(projected["card"]["trace_id"], r"^[a-f0-9]{32}$")
        self.assertNotIn(_SECRET, body.decode())
        self.assertNotIn(_SECRET, stderr.getvalue())
        ...
        with contextlib.redirect_stderr(stderr), mock.patch.object(
            ...
            status, _ct, body = wizard.start_premise_workspace(
                json.dumps({"workspace": "novelinit", "premise": "一句立意"}, ensure_ascii=False).encode(),
                "application/json",
            )
        self.assertEqual(status, 500)
        self.assertRegex(json.loads(body)["card"]["trace_id"], r"^[a-f0-9]{32}$")
        self.assertNotIn(_SECRET, body.decode())
        self.assertNotIn(_SECRET, stderr.getvalue())

        upload_body, upload_type = self._upload_body("pipeline")
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr), mock.patch.object(
            wizard.jobs, "start_job", side_effect=RuntimeError(_SECRET)
        ):
            status, _ct, body = wizard.start_upload(upload_body, upload_type)
        self.assertEqual(status, 500)
        self.assertRegex(json.loads(body)["card"]["trace_id"], r"^[a-f0-9]{32}$")
        self.assertNotIn(_SECRET, body.decode())
        self.assertNotIn(_SECRET, stderr.getvalue())
        ...
        with contextlib.redirect_stderr(stderr), mock.patch.object(
            ...
            status, _ct, body = wizard.start_premise_workspace(
                json.dumps(
                    {"workspace": "novel", "premise": "一句立意", "expand": True},
                    ensure_ascii=False,
                ).encode(),
                "application/json",
            )
        ...
        projected = json.loads(body)
        self.assertEqual(projected["expansion_error"], "设定扩写暂未启动，可稍后在工作台重试。")
        self.assertRegex(projected["expansion_trace_id"], r"^[a-f0-9]{32}$")
        self.assertNotIn(_SECRET, body.decode())
        self.assertNotIn(_SECRET, stderr.getvalue())
