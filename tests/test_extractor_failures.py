...
class ExtractorFailureIsolationTests(unittest.TestCase):
    ...
    def test_raise_on_failure_surfaces_batch_after_processing_all(self) -> None:
        # iter054c: raise_on_failure=True must (a) still process every chapter
        # (the one that succeeds is written to disk — batch not aborted mid-way),
        # then (b) raise ExtractionBatchFailure exposing the failed ids, so
        # rebuild-for-start aborts instead of bootstrapping a degraded base.
        with tempfile.TemporaryDirectory() as tmp:
            tmp = Path(tmp)
            fail_dir = tmp / "extraction_failures"
            extracted_dir = tmp / "extracted_jsons"
            ...
            with patch("src.extractor.load_manifest", return_value=manifest), patch(
                ...
                with self.assertRaises(ExtractionBatchFailure) as ctx:
                    extract_all(volume="all", force=True, raise_on_failure=True)

            # failure fact exposed to the caller
            self.assertEqual(ctx.exception.failed_ids, ["longzu_1_ch001"])
            self.assertEqual(ctx.exception.extracted, 1)
            # the chapter that DID succeed was still written (batch ran to the end)
            self.assertTrue((extracted_dir / "longzu_1_ch002.json").exists())
            self.assertTrue((fail_dir / "longzu_1_ch001.json").exists())
