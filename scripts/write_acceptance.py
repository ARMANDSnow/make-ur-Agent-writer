...
VERIFICATION_PROFILE = "canonical-novel-mock-offline"
CANONICAL_COMPLETED_STEPS = (
    "repository_state",
    "harness_check",
    "novel_only_boundary",
    "py_compile",
...
def finish_record(
    ...
    try:
        with _acceptance_lock(harness_fd):
            ...
            if status == "passed":
                if exit_code != 0:
                    raise ValueError("a passed acceptance record requires exit code 0")
                if steps != list(CANONICAL_COMPLETED_STEPS):
                    raise ValueError("passed acceptance requires the complete canonical step sequence")
                if failed_step:
                    raise ValueError("a passed acceptance record cannot name a failed step")
            payload.update(
                {
                    "status": status,
                    "exit_code": exit_code,
